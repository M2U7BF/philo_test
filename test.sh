...
if [ $arg_test -eq 1 ]; then
  put_test_name "引数エラーのチェック"

  put_test_pattern "引数の個数：少ない"
  philo 1
  philo 1 a
  philo 1 a a a

  put_test_pattern "引数の個数：多い"
  philo 1 1 1 1 1 1 1

  put_test_pattern "引数の値の種類：数値以外"
  philo 1 a a a a
  philo 1 a a a a a
  philo 1 1 a a a a
  philo 1 1 1 a a a
